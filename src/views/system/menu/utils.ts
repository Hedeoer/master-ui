...
/**
 * zetaTree to elementPlusTree
 *
 * 说明：
 * 因为zeta-kotlin项目定义的树形结构字段与element plus的树形结构不一致，故需要手动转换
 * @param menuTrees 树状结构的菜单数据
 * @param isLeaf 是否展开
 */
export function toTreeData(menuTrees?: SysMenu[], isLeaf = false): TreeData[] {
  ...
  return menuTrees.map((menu: SysMenu) => ({
    ...
    label: menu.label,
    field: menu.icon,
    children: toTreeData(menu.children, isLeaf),
    isLeaf,
